...
import styles from './selectionV2.module.scss';
    ...
    return (
        <>
            <div ref={ref} className={`justify-content-center w-100 ${styles.mainSec}`}>
                ...
                {
                    select && (
                        <div className={`d-block justify-content-center ${styles.selectionSec}`}>
                            {
                                props.options.map((item) => {
                                    return(
                                        <div className={`d-flex justify-content-center my-1 w-100`} key={item.value}>
                                            <div className={`text-start ${styles.optionsCell} ${value == item.value ? styles.optionSelected : ''}`} onClick={() => optionSelected(item.value)}>{item.content}</div>
                                        </div>
                                    )
                                })
                            }
